...
@Component({
  ...
  styleUrls: ['./mileage.component.less']
})
export class MileageComponent implements OnInit {
  ...
  getTableData() {
    ...
    this.loading = true;
    this._http.post('api/report/mileage/list', obj).subscribe((res: any) => {
      this.loading = false;
      if (res.success) {
  ...
  download() {
    ...
    this.ExcelService.download('api/report/mileage/list/export', obj, `车辆里程`);
  }
