...
@Component({
  ...
  styleUrls: ['./new-plan.component.less']
})
...
userInfo:any;
  ...
  getList() {
    ...
    this._http.post(`api/opsitem/category/v2`,params).subscribe((res: any) => {
      this.loading = false;
      if (res.success) {
  ...
  initModifyDataForm(id: any) {
    this._http.get(`api/opsitem/${id}/v2`).subscribe((res: any) => {
  ...
  handleData(num: number) {
    ...
    if (num == 1) {
      this._http.put('api/opsitem/v2', formOkData).subscribe((res: any) => {
      ...
      this._http.post('api/opsitem/v2', formOkData).subscribe((res: any) => {
        this.isOkLoading = false;
        if (res.success) {
  ...
  delete(id: any) {
    this.modal.info({
      ...
      nzTitle: '删除',
      nzOnOk: () => {
        this._http.delete(`api/opsitem/${id}/v2`).subscribe((res: any) => {
          if (res.success) {
