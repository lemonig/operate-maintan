...
userInfo:any;
  ngOnInit() {
    ...
    // 获取元数据
    this._http.get('api/opsitem/meta').subscribe((res: any) => {
      if (res.success) {
  ...
  getList() {
    ...
    this._http.post(`api/opsitem/category`,params).subscribe((res: any) => {
      this.loading = false;
      if (res.success) {
  ...
  initModifyDataForm(id: any) {
    this._http.get(`api/opsitem/${id}`).subscribe((res: any) => {
      this.modifyDataForm = this.fb.group({
        id: [res.data.id],
        category: [res.data.category, [Validators.required]],
        frequency: [res.data.frequency, [Validators.required]],
        name: [res.data.name, [Validators.required]],
        requirePhoto: [res.data.requirePhoto],
      })
  ...
  handleData(num: number) {
    ...
    if (num == 1) {
      this._http.put('api/opsitem', formOkData).subscribe((res: any) => {
      ...
      this._http.post('api/opsitem', formOkData).subscribe((res: any) => {
        this.isOkLoading = false;
        if (res.success) {
  ...
  delete(id: any) {
    this.modal.info({
      ...
      nzTitle: '删除',
      nzOnOk: () => {
        this._http.delete(`api/opsitem/${id}`).subscribe((res: any) => {
          if (res.success) {
