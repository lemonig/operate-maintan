...
@Component({
  ...
  styleUrls: ['./all-task.component.less']
})
export class AllTaskComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.activatedRoute.queryParams['value']['status']) {
      let routeVal = this.activatedRoute.queryParams['value']['status']
      this.status = parseInt(routeVal)
    }
    this.getTableData();
  ...
  getTableData() {
    ...
    this.loading = true;
    this._http.post('api/task/list', obj).subscribe((res: any) => {
      this.loading = false;
      if (res.success) {
        this.tableData = res.data.data;
        this.q.total = res.data.additional_data.total;
      } else {
        this.msg.error(res.message);
      }
    });
  ...
  download() {
    ...
    this.ExcelService.download('api/task/list/export', obj, `异常任务`);
  }
  confirm(id: any) {
    this.confirmId = id;
