...
@Component({
  selector: 'app-checking-city',
  templateUrl: './checking-city.component.html',
  styleUrls: ['./checking-city.component.less']
})
export class CheckingCityComponent implements OnInit {
  tableData: any[] = [];
  loading = false;
  q: any = {
    pageIndex: 1,
  ...
  getTableData() {
    ...
    this.loading = true;
    this._http.post('/api/opsitem/stat/region', params).subscribe((res: any) => {
      this.loading = false;
      if (res.success) {
