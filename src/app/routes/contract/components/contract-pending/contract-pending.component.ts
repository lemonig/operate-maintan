...
@Component({
  ...
  styleUrls: ['./contract-pending.component.less']
})
export class ContractPendingComponent implements OnInit {
  @Output() showRenewalPageEmitter: any = new EventEmitter<any>();
  @Output() showAddPageEmitter: any = new EventEmitter<any>();
  constructor(
  ...
  onNavigationEnd() {
    ...
    this.routerl.events.subscribe((event) => {
      ...
      if (event instanceof NavigationEnd) {
        ...
        *   this.$router.url 历史
        */
        // 在此跟距不同的路由进行不同的业务逻辑判断
        console.log(event.url);
        console.log(this.routerl.url);
  ...
  // renewal续签
  renewal(id: any) {
    this.showRenewalPageEmitter.emit(id);
    // this.targetId = id;
  ...
  // 结束合同
  finshContractForm: FormGroup
  finshContractVis: boolean = false
  handleFinshContract() {
    this.finshContractVis = true
    this.addDataForm = this.fb.group({
